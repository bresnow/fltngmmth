...
declare module "gun/types" {
  export interface IGunUserInstance {
    ...
    download(
      proof: string,
      size: number,
      cb: (progress: number, chunks: string) => void
    ): Promise<void>;
  ...
  if (who === "*") {
    ...
    user &&
      user
        ...
        .put(certificate, (ack) => {
          let ok = (ack as any).ok;
          if (ok) console.log("Friend Certificate created for: ", who);
        ...
        gun
          ...
          .put(
            ...
            (ack) => {
              ...
              if (err) console.log("Notification failed to: ", pub);
            },
            {
              opt: {
                cert,
...
Gun.chain.download = async function (proof, size, cb) {
  ...
  async function loop(i = 0, chunks: string[] = []) {
    ...
    let _chunk: string = await new Promise((resolve) =>
      gun.get(`${i}`).once((data) => {
        if (data && data[i]) {
          delete data._;
          resolve(data[i]);
