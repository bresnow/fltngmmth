//@ts-nocheck
import Base64Provider from 'js-base64'
...
import EnvVars from './utils/envar'
import dockerodeUtils from 'dockerode/lib/util'
import { log } from './utils/log'
import { DockerRegistryConfig, DockerAuthObj } from './DockerAuthObj'
...
class DockerApi {
    ...
    async getNodeIdByServiceName(
        ...
        const data = await self.dockerode
            .listTasks({
                filters: {
                    service: [serviceName],
    ...
    async pushImage(imageName: string, authObj: DockerAuthObj, buildLogs: BuildLog) {
        const self = this
    ...
    async ensureServiceConnectedToNetwork(serviceName: string, networkName: string) {
        const self = this
    ...
    async getLogForService(serviceName: string, tailCount: number, encoding: string) {
        const self = this
        return Promise.resolve() //
            .then(function () {
                return self.dockerode
